...
export function positionAlongJourney(waypoints: Waypoint[], t: number): { lat: number; lng: number } {
  if (waypoints.length === 0) return { lat: 0, lng: 0 };
  if (waypoints.length === 1) return { lat: waypoints[0].lat, lng: waypoints[0].lng };

  const clamped = Math.min(1, Math.max(0, t));
  const legDistances = waypoints.slice(1).map((wp, i) => haversineKm(waypoints[i], wp));
...
export function journeyCentroid(waypoints: Waypoint[]): { lat: number; lng: number } {
